...
DistanceConstraint::DistanceConstraint(float stiffness, float restLength)
	: restLength(restLength), Constraint(stiffness, CONSTRAINT_EQUALITY_TYPE::EQUALITY)
{}

float DistanceConstraint::projectConstraint(Particle* p[], float dt)
{
	...
	if (C != 0.0f)
	{
		...
		p[0]->x += stiffness * delX1 /** dt*/;
		p[1]->x += stiffness * delX2 /** dt*/;
	}
